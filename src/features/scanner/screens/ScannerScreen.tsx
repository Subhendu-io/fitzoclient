...
import { markAttendance, getTenantInfo } from '@/services/memberService';
...
export function ScannerScreen() {
  ...
  const handleBarCodeScanned = async ({ data }: { data: string }) => {
    ...
    try {
      ...
      Alert.alert('Scan Error', error.message || 'Failed to verify QR code');
      setScanned(false);
    } finally {
  ...
  const handleAction = async () => {
    ...
    try {
      if (scanResult.status === 'EXISTING_MEMBER') {
        ...
        Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);
        Alert.alert('Request Sent', 'Your request to join has been submitted.');
      } else if (scanResult.status === 'EXISTING_MEMBER' && profile?.activeGym === scanResult.tenantId) {
          // Attendance (if already a member)
          if (memberData) {
              await markAttendance(scanResult.tenantId, memberData.id, scanResult.branchId);
              Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);
              Alert.alert('Checked In!', `Welcome to ${scanResult.gymName}`);
          }
      }
      ...
      Alert.alert('Action Failed', error.message || 'Could not complete request');
    } finally {
      setIsProcessing(false);
    }
  };
