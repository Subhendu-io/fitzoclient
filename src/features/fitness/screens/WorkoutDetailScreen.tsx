  ...
  if (!assignment) {
    return (
      <ScreenWrapper className="bg-background items-center justify-center p-6">
        <Info {...({ size: 48, stroke: colors.muted } as any)} />
        <Text className="text-text text-lg font-kanit mt-4 text-center">Workout not found or unavailable.</Text>
      </ScreenWrapper>
    );
  }
    ...
    <ScreenWrapper className="bg-background">
      ...
      <ScrollView className="flex-1 px-6 pt-6" showsVerticalScrollIndicator={false}>
        ...
        >
          <View className="flex-row items-center mb-6">
            <View className="w-14 h-14 rounded-2xl bg-primary items-center justify-center mr-4">
              <Dumbbell {...({ size: 28, stroke: colors.onPrimary } as any)} />
            </View>
            <View className="flex-1">
              <Text className="text-text text-2xl font-bold font-kanit">{snapshot?.title || 'Workout Plan'}</Text>
              <View className="flex-row items-center mt-1">
                <CalendarIcon {...({ size: 12, stroke: colors.primary } as any)} />
          ...
          <View className="flex-row flex-wrap gap-4">
            {snapshot?.goal && (
              <View className="bg-white/5 px-4 py-2 rounded-xl flex-row items-center">
                <Trophy {...({ size: 14, stroke: colors.primary } as any)} />
                <Text className="text-text/80 text-xs font-kanit ml-2">{snapshot.goal}</Text>
              </View>
            )}
            {snapshot?.difficulty && (
              <View className="bg-white/5 px-4 py-2 rounded-xl flex-row items-center">
                <TrendingUp {...({ size: 14, stroke: colors.primary } as any)} />
                <Text className="text-text/80 text-xs font-kanit ml-2">{snapshot.difficulty}</Text>
              </View>
            )}
            {snapshot?.scheduleType && (
              <View className="bg-white/5 px-4 py-2 rounded-xl flex-row items-center">
                <Activity {...({ size: 14, stroke: colors.primary } as any)} />
                <Text className="text-text/80 text-xs font-kanit ml-2">{snapshot.scheduleType}</Text>
              </View>
            )}
          </View>
        </Animated.View>
        ...
        <View className="flex-row items-center mb-4">
          <Info {...({ size: 20, stroke: colors.primary } as any)} />
          <Text className="text-text text-lg font-bold font-kanit ml-2">Description</Text>
        </View>
        ...
        <Animated.View 
          ...
          className="bg-card p-6 rounded-[32px] border border-stone-200/5 dark:border-stone-900/5 mb-8"
        >
          <Text className="text-text/60 font-kanit leading-6">
        ...
        <View className="items-center py-10 opacity-30">
          <CheckCircle2 {...({ size: 48, stroke: colors.primary } as any)} />
