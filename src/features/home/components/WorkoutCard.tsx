...
export function WorkoutCard({ title, trainer, image }: any) {
  return (
    <TouchableOpacity className="mr-4 w-48 rounded-3xl overflow-hidden bg-card border border-white/5">
      <Image source={{ uri: image }} className="w-full h-40" />
      <View className="p-4">
        <Text className="text-white font-bold font-kanit">{title}</Text>
