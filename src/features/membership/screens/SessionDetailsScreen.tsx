  ...
  return (
    <ScreenWrapper className="bg-background">
      ...
      <ScrollView 
        ...
        refreshControl={
          <RefreshControl refreshing={refreshing} onRefresh={onRefresh} tintColor="#C8FF32" />
        }
      >
        {/* Active Plans Section */}
        <View className="flex-row items-center mt-6 mb-4">
          <Ticket {...({ size: 20, stroke: "#C8FF32" } as any)} />
          <Text className="text-white text-lg font-bold font-kanit ml-2">Active Plans</Text>
        </View>

        {loadingPlans ? (
          <ActivityIndicator color="#C8FF32" className="py-10" />
        ) : plans.length > 0 ? (
          <View className="space-y-4">
            {plans.map((plan, i) => (
              <Animated.View 
                ...
                className="bg-card p-6 rounded-[32px] border border-white/5"
              >
                <View className="flex-row justify-between items-start">
                  <View className="flex-1 mr-4">
                    <Text className="text-white text-xl font-bold font-kanit mb-1">{plan.planName}</Text>
          ...
          <View className="bg-card p-8 rounded-[32px] border border-white/5 items-center">
        ...
        <View className="flex-row items-center mt-10 mb-4">
          <HistoryIcon {...({ size: 20, stroke: "#C8FF32" } as any)} />
          <Text className="text-white text-lg font-bold font-kanit ml-2">Redemption History</Text>
        </View>
        ...
        {loadingHistory ? (
          ...
          <View className="space-y-4 pb-20">
            {history.map((item, i) => (
              <Animated.View 
                ...
                className="bg-card p-5 rounded-3xl border border-white/5 flex-row items-center"
              >
                <View className="w-10 h-10 rounded-2xl bg-white/5 items-center justify-center mr-4">
                   <User {...({ size: 18, stroke: "#C8FF32" } as any)} />
                </View>
                <View className="flex-1">
                   <Text className="text-white font-bold font-kanit mb-1">Trainer: {item.trainerName}</Text>
                   <View className="flex-row items-center">
                      <Clock {...({ size: 10, stroke: "#616161" } as any)} />
          ...
          <View className="bg-card p-8 rounded-[32px] border border-white/5 items-center mb-20">
